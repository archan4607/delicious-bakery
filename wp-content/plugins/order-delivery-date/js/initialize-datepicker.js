...
jQuery( document ).ready(function() {   
    ...
    jQuery(document).on( "ajaxComplete", function( event, xhr, options ) {
        ...
        } else if ( options.url.indexOf( "cart/?remove_item=" ) !== -1 || options.url.indexOf( "cart/?undo_item=" ) !== -1 ) {
            if( xhr.statusText != "abort" ) {
        ...
        if( update_settings == 'yes' ) {
            ...
            if( shipping_method.indexOf( 'local_pickup' ) === -1 ) {
                ...
                if ( 'on' === jQuery('#orddd_auto_populate_first_pickup_location').val() && ( ( '' === localStorage.getItem( "orddd_location_session" ) || undefined === localStorage.getItem( "orddd_location_session" ) ) || 'select_location' === localStorage.getItem( "orddd_location_session" ) ) ) {
            ...
            if( jQuery( "#orddd_enable_shipping_based_delivery" ).val() == "on" ) {
                ...
                if( ( new_billing_postcode != '' && new_billing_country != '' ) && 
                    ( false == is_shipping_checked || ( true == is_shipping_checked && '' != new_shipping_country && '' != new_shipping_postcode ) ) 
                    ) {
